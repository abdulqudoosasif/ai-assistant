...
const AssistantProfileSection = () => {
  ...
  return (
    <div className="w-72 bg-gray-100 dark:bg-gray-800 text-gray-800 m-2 dark:text-gray-200 p-6 rounded-lg shadow-md">
      <div className="flex flex-col items-center">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Your virtual helper</p>
      </div>
      <h4 className="text-sm font-bold mt-6">Recent Interactions</h4>
      <ul className="mt-3 space-y-3">
        {interactions.map((interaction, index) => (
          <li key={index} className="flex justify-between items-center bg-white dark:bg-gray-700 p-3 rounded-lg shadow-sm">
            <div>
              <p className="font-medium">{interaction.user}</p>
              <p className="text-sm text-gray-500 dark:text-gray-400">{interaction.query}</p>
            </div>
            <span className="text-xs text-gray-400 dark:text-gray-500">{interaction.time}</span>
          </li>
        ))}
      </ul>
    </div>
